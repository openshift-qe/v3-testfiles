#!/usr/bin/env bash
#test description: to add gitlab idp
...
step "Step 3: please input the client ID and secrete from gitlab:"
...
OUT=./oauth_gitlab.yaml

oc get oauth cluster -o yaml > $OUT
cat <<EOF >> $OUT
  - gitlab:
      clientID: 66eced85e87dbf9a476cb7d6e02159015126e7bbd3f5f29c6dfdf24acc3d6d24
      clientSecret:
        name: gitlab-secret
...
oc apply -f $OUT
sleep 1
oc create secret generic gitlab-secret --from-literal=clientSecret=$secret -n openshift-config
