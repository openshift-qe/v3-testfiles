...
function access_in_pod() {
    ...
    do 
        ...
        oc exec -t $pod_name -- bash -c "for ip in $pod_ips ; do curl --connect-timeout 1 \$ip:8080 ; done" >> pod_network_log
        oc exec -t $new_pod_name -- bash -c "for ip in $service_ips ; do curl --connect-timeout 1 \$ip:27017 ; done" >> pod_network_log
        echo >> pod_network_log
    done	
