...
- kind: ConfigMap
  ...
  data:
    rsyslog.conf: |+
      ...
      action(type="omfwd" target="rsyslogserver.openshift-logging.svc.cluster.local" port="514" protocol="tcp" queue.type="direct")
      #action(type="omfwd" target="rsyslogserver.openshift-logging.svc.cluster.local" port="514" protocol="udp" queue.type="direct")
      #action(type="omfwd" target="fd01::5:5cb6:62ff:fe00:b3" port="514" protocol="udp" queue.type="direct")
...
oc process -f  rsyslogserver.yaml -p NAME=rsyslogdebug | oc create -f -
