...
if [[ ${#RsyslogServiceNames[@]} > 0 ]]; then 
   ...
   oc set env ds/${fluentds} USE_REMOTE_SYSLOG=true REMOTE_SYSLOG_HOST=${RsyslogServiceNames[0]}  REMOTE_SYSLOG_TAG_KEY='ident,systemd.u.SYSLOG_IDENTIFIER'  REMOTE_SYSLOG_USE_RECORD=true REMOTE_SYSLOG_SEVERITY=info REMOTE_SYSLOG_TYPE=syslog
